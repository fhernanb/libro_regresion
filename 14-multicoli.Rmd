...
Mide la dispersión en el espectro de los valores propios de la matriz $\mathbf{X}^\top\mathbf{X}$. Se calcula como: $\kappa = \lambda_{\max} / \lambda_{\min}$. En $R$ se obtienen valores en raíz cuadrada, es decir, $\sqrt{\kappa}$, para el cual el criterio para detectar multicolinealidad es}
...
### Ejemplo {-}
Considere los datos del ejemplo anterior y calcule $\kappa$.

__Solución__

En el modelo ajustado `mod` está la matriz $\mathbf{X}$ que nos permite obtener $\kappa$. A continuación el código a usar.
...
De la salida anterior vemos que $\kappa$ es muy pero muy grande, eso significa que hay problemas de multicolinealidad, como era de esperarse.
